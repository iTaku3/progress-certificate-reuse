...
const out=oi<0?path.join(root,'実験結果/N1_run04'):path.resolve(args[oi+1]);
...
async function main(){
 ...
 const data=inputs(),result=[];save('inputs.json',data);
 const manifest={startedAt:new Date().toISOString(),node:process.version,nodeRed:require('node-red/package.json').version,httpNodeFile:httpFile,
